...
return {
  {
    ...
    opts = {
      providers = {
        ...
        googleai = { disable = false, secret = mia.secrets.gemini },
      },
    },
  },
  {
  ...
  },
  {
    ...
    config = function()
      ...
      local opts = { expr = true, nowait = true, silent = true, replace_keycodes = false }

      local imap = require('mapfun')('i')
      local plug = '<Plug>(Copilot%s)'

      imap(plug:format('Accept'), repeatAccept('Accept'), opts)
      imap(plug:format('AcceptWord'), repeatAccept('AcceptWord'), opts)
      imap(plug:format('AcceptLine'), repeatAccept('AcceptLine'), opts)

      imap(plug:format('Next'), vim.fn['copilot#Next'])
      imap(plug:format('Previous'), vim.fn['copilot#Previous'])
      imap(plug:format('Suggest'), vim.fn['copilot#Suggest'])
    end,
  },
}
